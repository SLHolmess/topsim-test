...
export function categoryPathBySlug(slug: any) {
  return `/bai-viet/categories/${slug}/`;
}
...
export async function getCategoryBySlug(slug: any) {
  ...
  const category: any = mapCategoryData(categoryData?.data.category);
...
export async function getCategories({ count }: any = {}) {
