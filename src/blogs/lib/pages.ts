...
export function pagePathBySlug(slug: any) {
  return `/${slug}`;
}
...
export async function getPageByUri(uri: any) {
  ...
  const page: any = [pageData?.data.page].map(mapPageData)[0];
...
const allPagesIncludesTypes: any = {
  all: QUERY_ALL_PAGES,
...
export async function getTopLevelPages(options: any) {
  const { pages } = await getAllPages(options);

  const navPages = pages.filter(({ parent }: any) => parent === null);

  // Order pages by menuOrder
  navPages.sort((a: any, b: any) => parseFloat(a.menuOrder) - parseFloat(b.menuOrder));
...
export function mapPageData(page = {}) {
  ...
  if (data.children) {
    data.children = data.children.edges.map(({ node }: any) => node);
  }
...
export function getBreadcrumbsByUri(uri: any, pages: any) {
  const breadcrumbs = [];
  const uriSegments = uri.split('/').filter((segment: any) => segment !== '');
  ...
  do {
    const breadcrumb = pages.find((page: any) => page.uri === `/${uriSegments.join('/')}/`);
