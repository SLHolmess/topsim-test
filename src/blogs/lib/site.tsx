...
export function constructPageMetadata(defaultMetadata: Record<string, any> = {}, pageMetadata: Record<string, any> = {}, options: Record<string, any> = {}) {
  const { router = {}, homepage = '', pathname = '' }: any = options;
  //   const { asPath } = router;
  // const asPath = window.location.pathname;
  // const url = `${homepage}/bai-viet${pathname}`;
...
export function helmetSettingsFromMetadata(metadata: Record<string, any> = {}, options: Record<string, any> = {}) {
  const { link = [], meta = [], setTitle = true }: any = options;
